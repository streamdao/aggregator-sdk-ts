...
import { StreamDaoProtocol } from "@streamdao/aggregator-sdk"
...
const streamdao = new StreamDaoProtocol(connection);
const secretKey = Uint8Array.from(privatekey);
const wallet = Keypair.fromSecretKey(secretKey);

const arbitrage = async () => {
    await streamdao.getTokenList()
    ...
    while (true) {
        const distributions = await streamdao.getRoutes({
            amount: amountIn,
